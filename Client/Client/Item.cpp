...
void CItem::Render()
{
	if (dynamic_cast<CPlayer*>(CObjectMgr::GetInstance()->GetPlayer())->GetInventoryState())
	{
		...
		D3DXMatrixScaling(&matScale, m_tInfo2.vSize.x / 1.25f, m_tInfo2.vSize.y / 1.25f, 0.f);
		D3DXMatrixTranslation(&matTrans, m_tInfo2.vPos.x, m_tInfo2.vPos.y, 0.f);
		m_tInfo2.matWorld = matScale * matTrans;
		...
		CDevice::GetInstance()->GetSprite()->SetTransform(&m_tInfo2.matWorld);
