...
_int CCaroline::Update(const _float & fTimeDelta)
{
...
void CCaroline::ChangeStateKey(DIR_ID eID)
{
	if (eID == DIR_ID_FORWORD)
		m_strStateKey = L"Caroline_Forward";
	else if (eID == DIR_ID_BACK)
		m_strStateKey = L"Caroline_Back";
	else if (eID == DIR_ID_LEFT)
		m_strStateKey = L"Caroline_Left";
	else if (eID == DIR_ID_RIGHT)
		m_strStateKey = L"Caroline_Right";
}

HRESULT CCaroline::Init(OBJECT_ID eID)
{
	m_tInfo.vSize = { 1.f,1.f,0.f };
	m_strObjectKey = L"Caroline";
	m_strStateKey = L"Caroline_Forward";
	...
	CObj* pObj = CAbstractFactory<CNameTag>::CreateObj(m_tInfo.vPos, L"Caroline");
	if (pObj == nullptr)
		return E_FAIL;
