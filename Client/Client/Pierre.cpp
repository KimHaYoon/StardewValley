...
HRESULT CPierre::Init()
{
	m_tInfo.vPos = { 100.f, 100.f,0.f };
	m_tInfo.vSize = { 1.f,1.f,0.f };
	m_strObjectKey = L"Pierre";
	m_strStateKey = L"Pierre_Forward";
	m_fSpeed = 10.f;
	m_tFrame = { 0.f, 4.f };
...
_int CPierre::Update(const _float & fTimeDelta)
{
...
void CPierre::LateUpdate(const _float & fTimeDelta)
{
	CObj::MoveFrame();
...
void CPierre::ChangeStateKey(DIR_ID eID)
{
	if (eID == DIR_ID_FORWORD)
		m_strStateKey = L"Pierre_Forward";
	else if (eID == DIR_ID_BACK)
		m_strStateKey = L"Pierre_Back";
	else if (eID == DIR_ID_LEFT)
		m_strStateKey = L"Pierre_Left";
	else if (eID == DIR_ID_RIGHT)
		m_strStateKey = L"Pierre_Right";
}

HRESULT CPierre::Init(OBJECT_ID eID)
{
	m_tInfo.vPos = { 100.f, 100.f,0.f };
	m_tInfo.vSize = { 1.f,1.f,0.f };
	m_strObjectKey = L"Pierre";
	...
	CObj* pObj = CAbstractFactory<CNameTag>::CreateObj(m_tInfo.vPos, L"Pierre");
	if (pObj == nullptr)
		return E_FAIL;
...
HRESULT CPierre::Init(LAYER_ID eID)
{
	m_tInfo.vPos = { 100.f, 100.f,0.f };
	m_tInfo.vSize = { 1.f,1.f,0.f };
	m_strObjectKey = L"Pierre";
