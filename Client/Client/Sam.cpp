...
HRESULT CSam::Init()
{
	...
	m_strObjectKey = L"Sam";
	m_strStateKey = L"Sam_Forward";
	m_fSpeed = 10.f;
	m_tFrame = { 0.f, 4.f };
...
_int CSam::Update(const _float & fTimeDelta)
{
...
void CSam::LateUpdate(const _float & fTimeDelta)
{
	CObj::MoveFrame();
...
void CSam::ChangeStateKey(DIR_ID eID)
{
	if (eID == DIR_ID_FORWORD)
		m_strStateKey = L"Sam_Forward";
	else if (eID == DIR_ID_BACK)
		m_strStateKey = L"Sam_Back";
	else if (eID == DIR_ID_LEFT)
		m_strStateKey = L"Sam_Left";
	else if (eID == DIR_ID_RIGHT)
		m_strStateKey = L"Sam_Right";
}

HRESULT CSam::Init(OBJECT_ID eID)
{
	m_tInfo.vPos = { 100.f, 100.f,0.f };
	m_tInfo.vSize = { 1.f,1.f,0.f };
	m_strObjectKey = L"Sam";
	...
	CObj* pObj = CAbstractFactory<CNameTag>::CreateObj(m_tInfo.vPos, L"Sam");
	if (pObj == nullptr)
		return E_FAIL;
...
HRESULT CSam::Init(LAYER_ID eID)
{
	m_tInfo.vPos = { 100.f, 100.f,0.f };
	m_tInfo.vSize = { 1.f,1.f,0.f };
	m_strObjectKey = L"Sam";
