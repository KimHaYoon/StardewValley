...
_int CRobin::Update(const _float & fTimeDelta)
{
...
void CRobin::LateUpdate(const _float & fTimeDelta)
{
	CObj::MoveFrame();
...
void CRobin::ChangeStateKey(DIR_ID eID)
{
	if (eID == DIR_ID_FORWORD)
		m_strStateKey = L"Robin_Forward";
	else if (eID == DIR_ID_BACK)
		m_strStateKey = L"Robin_Back";
	else if (eID == DIR_ID_LEFT)
		m_strStateKey = L"Robin_Left";
...
HRESULT CRobin::Init(OBJECT_ID eID)
{
	...
	CObj* pObj = CAbstractFactory<CNameTag>::CreateObj(m_tInfo.vPos, L"Robin");
	if (pObj == nullptr)
		return E_FAIL;
